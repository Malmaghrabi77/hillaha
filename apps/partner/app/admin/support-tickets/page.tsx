...
export default function SupportTicketsPage() {
  ...
  // Cleanup subscription on unmount or ticket change
  useEffect(() => {
    return () => {
  ...
  const loadTickets = async () => {
    try {
      ...
      // Load tickets with profile join
      const { data: ticketsData, error: ticketsError } = await (
        supabase as any
      )
      ...
      const openCount = enrichedTickets.filter(
        (t) => t.status === "open"
      ).length;
  ...
  const handleSendReply = async () => {
    if (!replyText.trim() || !selectedTicket || !auth.user) return;
