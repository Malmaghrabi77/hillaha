...
-- يشتغل من الصفر على قاعدة بيانات فارغة، وكمان آمن لو
-- الجداول موجودة أصلاً (كل عمليات idempotent)
...
CREATE TABLE IF NOT EXISTS public.profiles (
  ...
  role       text NOT NULL DEFAULT 'customer'
               CHECK (role IN ('customer','partner','driver','super_admin')),
...
ALTER TABLE public.partners
  ...
  ADD COLUMN IF NOT EXISTS category        text NOT NULL DEFAULT 'مطاعم',
  ADD COLUMN IF NOT EXISTS type            text NOT NULL DEFAULT 'restaurant',
  ADD COLUMN IF NOT EXISTS city            text DEFAULT 'قنا',
...
-- sync old available → is_available
DO $$ BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
